...
int all_express_delivered() {
    for (int i = 0; i < order_count; i++) {
        if (strcmp(shared_orders[i].type, "Express") == 0 && shared_orders[i].delivered == 0) {
            return 0;  // If there are still undelivered express orders
        }
    }
    return 1;  // All express orders delivered
}
    ...
    while (1) {
        ...
        for (int i = 0; i < order_count; i++) {
            if (shared_orders[i].delivered == 0 && strcmp(shared_orders[i].type, "Express") == 0) {
                ...
                all_delivered = 0;  // Still some orders left to deliver
            }
        }
        
        if (all_delivered) {
            printf("All express orders have been delivered. Exiting...\n");
            exit(0);  // Exit the program if all express orders are delivered
        }
